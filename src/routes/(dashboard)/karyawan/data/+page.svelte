<script lang="ts">
  ...
  let filteredList = $derived.by(() => {
    ...
    if (filterTipe !== "semua") list = list.filter((k) => k.tipe_akun === filterTipe);
    if (searchQuery.trim()) {
  ...
  // ── Dialog helpers ──────────────────────────────────────────────────
  function bukaTambah() {
    fNama = ""; fEmail = ""; fPassword = "";
    fRole = "kepala_jahit"; fTipe = "permanent"; fExpired = "";
    openTambah = true;
  }
  ...
  async function submitTambah() {
    ...
    try {
      await createAkunKaryawan({
        ...
        role: fRole,
        tipe_akun: fTipe,
        tanggal_expired: fTipe === "temporary" && fExpired ? new Date(fExpired) : null,
      });
      openTambah = false;
  ...
  async function submitEdit() {
    ...
    try {
      await updateKaryawan(selectedKaryawan.uid, {
        ...
        role: eRole,
        tipe_akun: eTipe,
        tanggal_expired: eTipe === "temporary" && eExpired ? new Date(eExpired) : null,
      });
      openEdit = false;
...
<div class="mb-5 flex flex-wrap items-start justify-between gap-4">
  <div>
    <div class="flex items-center gap-2">
      <a href="/karyawan" class="text-sm text-gray-400 hover:text-gray-600 hover:underline">Dashboard Karyawan</a>
    ...
    <p class="mt-0.5 text-sm text-gray-500">Kelola akun dan kontrak karyawan</p>
  </div>
    ...
    <div class="space-y-0">
      {#each Array(5) as _}
        <div class="flex items-center gap-4 border-b border-gray-50 px-5 py-4">
          ...
          <div class="ml-auto h-5 w-20 animate-pulse rounded-full bg-gray-100"></div>
  ...
  {:else if filteredList.length === 0}
    <div class="flex flex-col items-center justify-center gap-3 py-16">
      ...
      {#if searchQuery || filterTipe !== "semua"}
        <p class="text-sm font-medium text-gray-500">Tidak ada karyawan yang cocok</p>
        <Button variant="link" size="sm" onclick={() => { searchQuery = ""; filterTipe = "semua"; }}>
          Hapus filter
        </Button>
      {:else}
        <p class="text-sm font-medium text-gray-500">Belum ada akun karyawan</p>
        <p class="text-xs text-gray-400">Mulai dengan menambahkan akun karyawan pertama</p>
          ...
          <Table.Row>
            ...
            <Table.Cell class="text-center">
              {#if k.tipe_akun === "temporary"}
                <span class="inline-block rounded-full bg-orange-100 px-2.5 py-0.5 text-xs font-semibold text-orange-700">
                  Temporary
                </span>
              {:else}
                <span class="inline-block rounded-full bg-green-100 px-2.5 py-0.5 text-xs font-semibold text-green-700">
                  Permanent
                </span>
              {/if}
            </Table.Cell>
            <Table.Cell>
              {#if k.tipe_akun === "temporary" && k.tanggal_expired}
                <span class="text-xs {expired ? 'font-semibold text-red-600' : 'text-gray-500'}">
            ...
            <Table.Cell class="text-right">
              <div class="flex justify-end gap-2">
                <Button variant="outline" size="sm" onclick={() => bukaEdit(k)}>
...
<Dialog.Root bind:open={openTambah}>
  <Dialog.Content class="max-w-md">
    <Dialog.Header>
      ...
      <Dialog.Description>Buat akun login baru untuk karyawan.</Dialog.Description>
    </Dialog.Header>

    <div class="space-y-4">
      <div class="space-y-1.5">
        <label class="mb-1.5 block text-sm font-medium text-gray-700" for="t-nama">
          Nama Lengkap <span class="text-red-500">*</span>
        </label>
        <Input id="t-nama" placeholder="cth: Budi Santoso" bind:value={fNama} />
      </div>
      <div class="space-y-1.5">
        <label class="mb-1.5 block text-sm font-medium text-gray-700" for="t-email">
          Email <span class="text-red-500">*</span>
        </label>
        <Input id="t-email" type="email" placeholder="budi@zarqa.id" bind:value={fEmail} />
      </div>
      <div class="space-y-1.5">
        <label class="mb-1.5 block text-sm font-medium text-gray-700" for="t-password">
          Password <span class="text-red-500">*</span>
          <span class="text-xs font-normal text-gray-400">(minimal 6 karakter)</span>
        </label>
        <Input id="t-password" type="password" placeholder="••••••••" bind:value={fPassword} />
      </div>
      <div class="space-y-1.5">
      ...
      {#if fTipe === "temporary"}
        <div class="space-y-1.5">
          <label class="mb-1.5 block text-sm font-medium text-gray-700" for="t-expired">
...
<Dialog.Root bind:open={openEdit}>
  <Dialog.Content class="max-w-md">
    <Dialog.Header>
      ...
      <Dialog.Description>Perbarui nama, role, atau tipe kontrak. Email tidak dapat diubah.</Dialog.Description>
    </Dialog.Header>
    ...
    {#if selectedKaryawan}
      <div class="space-y-4">
        <div class="rounded-lg border border-gray-100 bg-gray-50 px-3 py-2.5">
          ...
          <p class="text-sm font-medium text-gray-700">{selectedKaryawan.email}</p>
        </div>
        <div class="space-y-1.5">
          <label class="mb-1.5 block text-sm font-medium text-gray-700" for="e-nama">
            Nama Lengkap <span class="text-red-500">*</span>
          </label>
        ...
        <div class="space-y-1.5">
          <label class="mb-1.5 block text-sm font-medium text-gray-700">Tipe Akun</label>
          <div class="flex gap-3">
            <button
              type="button"
        ...
        {#if eTipe === "temporary"}
          <div class="space-y-1.5">
            <label class="mb-1.5 block text-sm font-medium text-gray-700" for="e-expired">
              Tanggal Berakhir Kontrak <span class="text-red-500">*</span>
            </label>
            <input
              id="e-expired"
  ...
  <Dialog.Content class="max-w-sm">
    ...
    {#if selectedKaryawan}
      <div class="rounded-lg border border-gray-100 bg-gray-50 p-4">
        <p class="text-sm font-semibold text-gray-800">{selectedKaryawan.name}</p>
        <p class="text-xs text-gray-400">{selectedKaryawan.email}</p>
        <span class="mt-2 inline-block rounded-full bg-blue-50 px-2 py-0.5 text-xs font-medium text-blue-700">

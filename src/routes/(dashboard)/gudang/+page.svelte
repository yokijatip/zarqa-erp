<script lang="ts">
  ...
  const STAGES: StageConf[] = [
    ...
    { status: 'CUTTING_DONE',        label: 'Cutting Selesai', short: 'Cut ✓',    dot: 'bg-yellow-500',  ring: 'ring-yellow-300',  textActive: 'text-yellow-700',  bgActive: 'bg-yellow-50',  borderActive: 'border-yellow-200'  },
  ...
  // ── Quick actions ─────────────────────────────────────────────────
  const ACTIONS = [
    { title: 'Buat Order Produksi', desc: 'Mulai batch produksi baru',      href: '/order-produksi', iconBg: 'bg-blue-100',   iconColor: 'text-blue-600',   svg: 'M12 4.5v15m7.5-7.5h-15' },
    { title: 'Tambah Stok Kain',    desc: 'Catat kain masuk ke gudang',     href: '/stok-kain',      iconBg: 'bg-amber-100',  iconColor: 'text-amber-600',  svg: 'M6.429 9.75 2.25 12l4.179 2.25m0-4.5 5.571 3 5.571-3m-11.142 0L2.25 7.5 12 2.25l9.75 5.25-4.179 2.25m0 0L21.75 12l-4.179 2.25m0 0 4.179 2.25L12 21.75 2.25 16.5l4.179-2.25m11.142 0-5.571 3-5.571-3' },
    { title: 'Model Baju',          desc: 'Kelola katalog model produksi',   href: '/model-baju',     iconBg: 'bg-pink-100',   iconColor: 'text-pink-600',   svg: 'M15.75 10.5V6a3.75 3.75 0 1 0-7.5 0v4.5m11.356-1.993 1.263 12c.07.665-.45 1.243-1.119 1.243H4.25a1.125 1.125 0 0 1-1.12-1.243l1.264-12A1.125 1.125 0 0 1 5.513 7.5h12.974c.576 0 1.059.435 1.119 1.007Z' },
    { title: 'Barang Jadi',         desc: 'Lihat stok barang siap kirim',   href: '/barang-jadi',    iconBg: 'bg-teal-100',   iconColor: 'text-teal-600',   svg: 'm20.25 7.5-.625 10.632a2.25 2.25 0 0 1-2.247 2.118H6.622a2.25 2.25 0 0 1-2.247-2.118L3.75 7.5m8.25 3v6.75m0 0-3-3m3 3 3-3M3.375 7.5h17.25c.621 0 1.125-.504 1.125-1.125v-1.5c0-.621-.504-1.125-1.125-1.125H3.375c-.621 0-1.125.504-1.125 1.125v1.5c0 .621.504 1.125 1.125 1.125Z' },
    { title: 'Catat Barang Keluar', desc: 'Rekam pengiriman barang jadi',   href: '/barang-keluar',  iconBg: 'bg-green-100',  iconColor: 'text-green-600',  svg: 'M8.25 18.75a1.5 1.5 0 0 1-3 0m3 0a1.5 1.5 0 0 0-3 0m3 0h6m-9 0H3.375a1.125 1.125 0 0 1-1.125-1.125V14.25m17.25 4.5a1.5 1.5 0 0 1-3 0m3 0a1.5 1.5 0 0 0-3 0m3 0h1.125c.621 0 1.129-.504 1.09-1.124a17.902 17.902 0 0 0-3.213-9.193 2.056 2.056 0 0 0-1.58-.86H14.25M16.5 18.75h-2.25m0-11.177v-.958c0-.568-.422-1.048-.987-1.106a48.554 48.554 0 0 0-10.026 0 1.106 1.106 0 0 0-.987 1.106v7.635m12-6.677v6.677m0 4.5v-4.5m0 0h-12' },
    { title: 'Order Produksi',      desc: 'Lihat semua order & statusnya',  href: '/order-produksi', iconBg: 'bg-purple-100', iconColor: 'text-purple-600', svg: 'M9 12h3.75M9 15h3.75M9 18h3.75m3 .75H18a2.25 2.25 0 0 0 2.25-2.25V6.108c0-1.135-.845-2.098-1.976-2.192a48.424 48.424 0 0 0-1.123-.08m-5.801 0c-.065.21-.1.433-.1.664 0 .414.336.75.75.75h4.5a.75.75 0 0 0 .75-.75 2.25 2.25 0 0 0-.1-.664m-5.8 0A2.251 2.251 0 0 1 13.5 2.25H15c1.012 0 1.867.668 2.15 1.586m-5.8 0c-.376.023-.75.05-1.124.08C9.095 4.01 8.25 4.973 8.25 6.108V8.25m0 0H4.875c-.621 0-1.125.504-1.125 1.125v11.25c0 .621.504 1.125 1.125 1.125h9.75c.621 0 1.125-.504 1.125-1.125V9.375c0-.621-.504-1.125-1.125-1.125H8.25Z' },
  ];
  ...
  let antriCount  = $derived(batchAktif.filter(b => b.status === 'PENDING_CUTTING').length);
  let antriPcs    = $derived(batchAktif.filter(b => b.status === 'PENDING_CUTTING').reduce((s, b) => s + b.total_pcs, 0));

  let siapKirimCount = $derived(batchAktif.filter(b => b.status === 'STEAM_DONE').length);
  let siapKirimPcs   = $derived(batchAktif.filter(b => b.status === 'STEAM_DONE').reduce((s, b) => s + b.total_pcs, 0));

  let terlambatCount = $derived(batchAktif.filter(b => hitungHari(b.createdAt) > 5).length);

  let filteredBatch = $derived.by(() => {
    let list = batchAktif;
    if (selectedStage) list = list.filter(b => b.status === selectedStage);
    return [...list].sort((a, b) => hitungHari(b.createdAt) - hitungHari(a.createdAt));
  });
  ...
  function pcsStatus(status: StatusBatch): number {
    return batchAktif.filter(b => b.status === status).reduce((s, b) => s + b.total_pcs, 0);
  }
...
<div class="mb-5 flex flex-wrap items-start justify-between gap-3">
  ...
  <div class="flex items-center gap-2">
    {#if lastUpdated}
      <span class="text-xs text-gray-400">Update: {formatLastUpdated(lastUpdated)}</span>
    {/if}
...
{#if !loadingBatch && terlambatCount > 0}
  <div class="mb-4 flex items-start gap-3 rounded-xl border border-red-200 bg-red-50 p-4">
    ...
    <div class="min-w-0 flex-1">
      <p class="text-sm font-semibold text-red-800">Ada Batch Melebihi 5 Hari</p>
      <p class="mt-0.5 text-xs text-red-700">
        {terlambatCount} batch berjalan lebih dari 5 hari. Segera periksa progres produksi.
      </p>
    </div>
    <a href="/order-produksi" class="shrink-0 rounded-lg bg-red-600 px-3 py-1.5 text-xs font-semibold text-white transition hover:bg-red-700">
      Cek Order →
    </a>
  </div>
{/if}
...
<div class="mb-4 grid grid-cols-2 gap-4 lg:grid-cols-4">
  ...
  <div class="rounded-xl border border-gray-100 bg-white p-5 shadow-sm">
    ...
    {#if loadingBatch}
      <div class="mt-3 h-8 w-16 animate-pulse rounded bg-gray-100"></div>
      <div class="mt-1.5 h-3 w-32 animate-pulse rounded bg-gray-100"></div>
    {:else}
      ...
      {#if terlambatCount > 0}
        <p class="mt-3 text-xs font-medium text-red-500">{terlambatCount} batch &gt;5 hari ⚠</p>
      {:else}
        <p class="mt-3 text-xs text-gray-400">Semua batch dalam jadwal</p>
      {/if}
    {/if}
  </div>
  ...
  <div class="rounded-xl border border-gray-100 bg-white p-5 shadow-sm">
    ...
    {#if loadingBatch}
      <div class="mt-3 h-8 w-16 animate-pulse rounded bg-gray-100"></div>
      <div class="mt-1.5 h-3 w-28 animate-pulse rounded bg-gray-100"></div>
    {:else}
      <p class="mt-3 text-3xl font-bold tracking-tight text-gray-900">{antriCount}</p>
      <p class="mt-0.5 text-xs text-gray-500">{antriPcs.toLocaleString('id-ID')} pcs menunggu</p>
      <p class="mt-3 text-xs text-gray-400">Cutting aktif: {countStatus('CUTTING_IN_PROGRESS')} batch</p>
    {/if}
  </div>

  <!-- Siap Kirim -->
  <div class="rounded-xl border {siapKirimCount > 0 ? 'border-emerald-200 bg-emerald-50' : 'border-gray-100 bg-white'} p-5 shadow-sm">
    <div class="flex items-start justify-between">
      <p class="text-xs font-medium uppercase tracking-wider {siapKirimCount > 0 ? 'text-emerald-600' : 'text-gray-400'}">Siap Kirim</p>
      <span class="flex h-7 w-7 items-center justify-center rounded-lg {siapKirimCount > 0 ? 'bg-emerald-100' : 'bg-gray-100'}">
        <svg class="h-3.5 w-3.5 {siapKirimCount > 0 ? 'text-emerald-600' : 'text-gray-400'}" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor">
    ...
    {#if loadingBatch}
      ...
      <div class="mt-1.5 h-3 w-28 animate-pulse rounded bg-gray-100"></div>
    {:else}
      <p class="mt-3 text-3xl font-bold tracking-tight {siapKirimCount > 0 ? 'text-emerald-700' : 'text-gray-900'}">{siapKirimCount}</p>
      <p class="mt-0.5 text-xs {siapKirimCount > 0 ? 'text-emerald-600' : 'text-gray-500'}">
        {siapKirimPcs.toLocaleString('id-ID')} pcs menunggu dicatat
      </p>
      {#if siapKirimCount > 0}
  ...
  <div class="rounded-xl border {kainKritis.length > 0 ? 'border-amber-200 bg-amber-50' : 'border-gray-100 bg-white'} p-5 shadow-sm">
    ...
    {:else}
      ...
      {:else}
        <p class="mt-3 text-xs text-gray-400">{stokKainList.length} jenis kain tersedia</p>
      {/if}
    {/if}
  </div>
  ...
  <div class="mb-4 flex items-center justify-between">
    <div>
      ...
      <p class="text-xs text-gray-400">
        ...
        {#if loadingBatch}
          <span class="inline-block h-3 w-8 animate-pulse rounded bg-gray-100 align-middle"></span>
        {:else}
    ...
    {#each STAGES as stage}
      ...
      >
        ...
        {:else}
          ...
          {#if active}
            <span class="text-[10px] opacity-70 {stage.textActive}">{pcs} pcs</span>
          {/if}
        {/if}
      </button>
    {/each}
      ...
      <div class="space-y-1.5">
        {#each Array(4) as _}
          <div class="flex items-center gap-3 rounded-lg border border-gray-100 bg-gray-50 px-3 py-2.5">
            ...
            <div class="ml-auto h-3.5 w-12 animate-pulse rounded bg-gray-200"></div>
    ...
    {:else if filteredBatch.length === 0}
      <div class="flex flex-col items-center gap-2 py-8">
        ...
        <p class="text-sm text-gray-400">
          {selectedStage ? 'Tidak ada batch pada tahap ini' : 'Belum ada batch produksi aktif'}
        </p>
        {#if !selectedStage}
            ...
            <div class="flex min-w-0 items-center gap-3">
              ...
              <div class="min-w-0">
                ...
                <p class="text-[11px] text-gray-400">
                  {batch.detail_ukuran.map(d => `${d.ukuran}:${d.jumlah_pcs}`).join(' · ')}
                </p>
              </div>
            </div>
  ...
  <div class="rounded-xl border border-gray-100 bg-white p-5 shadow-sm">
    ...
    {:else if stokKainList.length === 0}
      <div class="flex flex-col items-center gap-2 py-8">
        ...
        <a href="/stok-kain" class="text-xs font-medium text-blue-600 hover:underline">Tambah kain pertama →</a>
      </div>
    {:else}
      <div class="space-y-3">
          ...
          <div>
            <div class="mb-1.5 flex items-center justify-between">
              <div class="flex items-center gap-1.5">
                <span class="h-1.5 w-1.5 rounded-full {kritis ? 'bg-amber-400' : 'bg-blue-400'}"></span>
        ...
        {#if stokKainList.length > 7}
          <p class="pt-1 text-center text-xs text-gray-400">
            ...
            <a href="/stok-kain" class="text-blue-500 hover:underline">lihat semua</a>
          </p>
        {/if}
      </div>
    {/if}
